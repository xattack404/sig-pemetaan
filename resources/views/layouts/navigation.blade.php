        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <x-nav-dropdown title="Apps" align="right" width="48">
                        ...
                        <x-dropdown-link href="{{ route('detail-transaksis.index') }}">
                        Detail Transaksis
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\ItemTransaksiPetani::class)
                        <x-dropdown-link href="{{ route('item-transaksi-petanis.index') }}">
                        Item Transaksi Petanis
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\JenisTanamans::class)
                        <x-dropdown-link href="{{ route('all-jenis-tanamans.index') }}">
                        All Jenis Tanamans
                        </x-dropdown-link>
                        @endcan
                        ...
                        @endcan
                        @can('view-any', App\Models\TransaksiPetani::class)
                        <x-dropdown-link href="{{ route('transaksi-petanis.index') }}">
                        Transaksi Petanis
                        </x-dropdown-link>
                        @endcan
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                ...
                @can('view-any', App\Models\Barang::class)
                <x-responsive-nav-link href="{{ route('barangs.index') }}">
                Barangs
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\DetailTransaksi::class)
                <x-responsive-nav-link href="{{ route('detail-transaksis.index') }}">
                Detail Transaksis
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\ItemTransaksiPetani::class)
                <x-responsive-nav-link href="{{ route('item-transaksi-petanis.index') }}">
                Item Transaksi Petanis
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\JenisTanamans::class)
                <x-responsive-nav-link href="{{ route('all-jenis-tanamans.index') }}">
                All Jenis Tanamans
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\Lahan::class)
                <x-responsive-nav-link href="{{ route('lahans.index') }}">
                Lahans
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\LimitStok::class)
                <x-responsive-nav-link href="{{ route('limit-stoks.index') }}">
                Limit Stoks
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\Panen::class)
                <x-responsive-nav-link href="{{ route('panens.index') }}">
                Panens
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\Transaksi::class)
                <x-responsive-nav-link href="{{ route('transaksis.index') }}">
                Transaksis
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\TransaksiPetani::class)
                <x-responsive-nav-link href="{{ route('transaksi-petanis.index') }}">
                Transaksi Petanis
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\User::class)
